...
internal class OptionCurrentTypeSelector : IOptionTypeSelector, IImplementationTypeSelector, ISelector {
    ...
    public OptionCurrentTypeSelector(IImplementationTypeSelector inner, IEnumerable<Type> types) {
        this._Inner = inner;
        this._Types = types;
    }
    ...
    public IOptionTypeSelector AddOptionsCurrent() {
        ...
        foreach (var type in this._Types) {
            var attr = type.GetCustomAttribute<OptionsCurrentAttribute>();
            if (attr is null) {
                continue;
            }
            listOptionPopulation.Add(new OptionPopulation(type, true, attr.Section));
        }
        return this;
    }
